...
v0.4.3 不要求修改 HomePage、Docker Socket Proxy、共享网络或 MySQL。首次启动会自动把 v0.4.0-v0.4.2 的重复 Docker 发现配置迁移成 metadata-only 模型：`docker.yaml` 保留唯一连接配置，`admin-settings.json` 只保留显示名称、Public Host 和必要的 Discovery Override。
