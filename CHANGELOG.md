...
- 修复两列字段因“来源”说明存在与否造成的输入框纵向错位；向导字段统一固定输入区与来源说明区高度。
- 新增服务类型识别与置信度提示。
- Docker 发现页新增“智能推荐（按服务类型）”分组策略；可继续手动固定分组，Homepage Label 优先级最高。
- 为常见服务增加类型到现有 Homepage 分组的推荐规则。
- 扩展常见服务图标推断，并让卡片预览直接尝试显示 URL / `sh-*` 图标，失败时自动回退文字。

### 备份回滚

- 新增可视化备份保留上限设置，范围 1–500 组。
- UI 设置持久化到 `/data/admin-settings.json`，环境变量 `BACKUP_LIMIT` 作为默认值。
- 降低上限后立即清理最旧的超额备份。
- 新增“恢复默认”操作，恢复环境变量定义的上限。

### 兼容性

- 延续 v0.2.2 的主题菜单、备份删除、Docker Proxy 和敏感字段保护。
- 已完成 v0.2.1+ Docker Proxy 迁移的部署不需要再次修改 HomePage Stack。
