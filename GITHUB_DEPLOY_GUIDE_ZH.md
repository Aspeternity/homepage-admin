...
v0.5.4 起默认部署只需要一个 `homepage-admin` 容器：

```text
homepage-admin
├── /config -> Homepage 配置目录（宿主机 Bind Mount）
└── /data   -> Admin 持久化数据（推荐 ./data:/data）
```
...
默认 Compose 显式使用 `network_mode: bridge`，端口只写 `3001:3001`。Docker 发现属于可选功能；如需使用，在目标 Docker 主机单独部署只读 Socket Proxy，并使用 `2375:2375` 映射端口，再通过 IP / DNS 从“Docker 主机管理”添加即可。
...
3. 上传解压后的普通项目文件进行覆盖。
4. Commit message：`Release v0.5.4`。
5. 打开 **Actions**。
...
创建 Tag `v0.5.4` 时，会生成类似：

```text
ghcr.io/aspeternity/homepage-admin:0.5.4
